...
async function fetchCheongyak() {
  ...
  const prompt = `당신은 부동산 청약 정보 수집 봇입니다.

[중요] 반드시 JSON만 출력하세요. 설명, 사과, 마크다운 절대 금지. JSON 외 텍스트가 있으면 시스템 오류가 발생합니다.

${today} 기준으로 아래 지역의 아파트 청약 공고를 웹 검색하세요:
- 성남시 (분당, 판교, 위례, 복정)
- 용인시 수지구
- 하남시 (미사, 감일)

검색 대상: LH청약플러스, 청약홈, 부동산 뉴스

현재 진행중인 공고가 없더라도, 예정된 공고나 최근 분양한 단지 정보를 반드시 포함하세요. items 배열이 비어있으면 안 됩니다.

JSON 형식:
{
  "summary": "${today} 기준 성남·수지·하남 청약 현황 요약 2~3문장",
  "items": [
    {
      "name": "단지명 (정확한 공식 명칭)",
      ...
      "area": "성남|분당|판교|위례|복정|수지|미사|감일|하남 중 가장 가까운 곳",
...
규칙:
1. 마감된 공고도 최근 1개월 이내면 포함
2. 예정 공고는 최대한 많이 포함
3. 현재 공고가 적으면 2026~2027년 분양 예정 단지도 포함
4. 반드시 { 로 시작하고 } 로 끝나는 유효한 JSON만 출력`;
